...
<Note>
  Visual identity comes from **`docs/docs.json`** (theme, palette, typography, logos) plus optional **`docs/style.css`**. Hosted assets live next to configuration (for example `docs/logo/light.svg`).
  For authoring assistance in AI tools that support skills, Mintlify publishes discovery at **`https://www.mintlify.com/docs`**:  
  `npx skills add https://www.mintlify.com/docs`  
  (Use the `www` host; `https://mintlify.com/docs` may not expose the `.well-known` skills endpoint.)
</Note>
